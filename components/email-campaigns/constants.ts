/**
 * Email Campaigns — constants and UI helpers (status/category labels and colors)
 */

import type { EmailStats } from "./types";

export const DEFAULT_STATS: EmailStats = {
  totalCampaigns: 0,
  totalSent: 0,
  totalDelivered: 0,
  totalFailed: 0,
...
export const EMAIL_ERROR_MESSAGES_AR: Record<string, string> = {
  ...
  "No recipients to restart. Provide customer_ids or manual_phones, or ensure campaign was sent with recipients.":
    "لا يوجد مستلمون لإعادة الإرسال. حدّد عملاء أو عناوين بريد، أو تأكد أن الحملة أُرسلت سابقاً بمستلمين.",
};

export const EMAIL_ERROR_CODES_AR: Record<string, string> = {
...
export function getEmailErrorAr(message: string): string {
  const trimmed = message?.trim() ?? "";
  return EMAIL_ERROR_MESSAGES_AR[trimmed] ?? trimmed;
}
...
export function getEmailCreditNoteAr(note: string | undefined): string | undefined {
