...
import type { UpdateCampaignBody, SendCampaignBody, ResumeCampaignBody } from "@/lib/services/email-api";
import type { EmailCampaign } from "../types";
import { mapApiCampaignToUI } from "../types";
import { EMAIL_USER_MESSAGES, getEmailCreditNoteAr } from "../constants";
  ...
  const handlePauseCampaign = useCallback(
    async (id: string) => {
      try {
        ...
        toast.success(getEmailCreditNoteAr(note) ?? note ?? EMAIL_USER_MESSAGES.afterPause(
          data.sent_count,
  ...
  const handleResumeCampaign = useCallback(
    async (id: string, params: ResumeCampaignBody) => {
      ...
      try {
        ...
        const note = data.credit_info?.note;
        const message = getEmailCreditNoteAr(note) ?? note
          ?? (data.mode === "continue"
            ? EMAIL_USER_MESSAGES.afterResumeContinue(data.recipient_count)
            : EMAIL_USER_MESSAGES.afterResumeRestart(data.recipient_count));
