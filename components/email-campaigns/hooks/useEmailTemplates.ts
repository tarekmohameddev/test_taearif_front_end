...
import { getTemplates, deleteTemplate, updateTemplate } from "@/lib/services/email-api";
import type { UpdateTemplateBody } from "@/lib/services/email-api";
import type { EmailTemplate } from "../types";
import { mapApiTemplateToUI } from "../types";
import { getEmailErrorAr } from "../constants";
...
export function useEmailTemplates() {
  ...
  const fetchTemplates = useCallback(async () => {
    ...
    try {
      ...
      setError(msg);
      toast.error(getEmailErrorAr(msg));
    } finally {
      setLoading(false);
    }
  }, []);
