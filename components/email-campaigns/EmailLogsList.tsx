...
export function EmailLogsList({ logs, loading, error }: EmailLogsListProps) {
      ...
      <CardHeader>
        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <CardDescription>عرض تفصيلي لجميع رسائل البريد الإلكتروني المرسلة</CardDescription>
          </div>
              ...
              >
                <div className="flex items-start gap-3 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground mb-2 dir-ltr">{log.recipientEmail}</p>
                    <p className="text-sm font-medium text-muted-foreground mb-1">الموضوع: {log.subject}</p>
          ...
          <div className="rounded-md border overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {logs.map((log) => (
                  <TableRow key={log.id}>
                    <TableCell className="font-medium text-right">{log.recipientName || "—"}</TableCell>
                    <TableCell className="text-right dir-ltr">{log.recipientEmail}</TableCell>
                    <TableCell className="text-right max-w-[140px] truncate" title={log.subject}>
                      {log.subject}
                    </TableCell>
