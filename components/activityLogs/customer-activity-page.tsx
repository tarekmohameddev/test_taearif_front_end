...
export default function CustomerActivityLogPage() {
  ...
  const fetchCustomerLogs = async () => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get<CustomerLogsResponse>(
        `/v1/customers/${customerId}/logs`,
        {
          params: {
            page: currentPage,
            search: searchQuery,
      ...
      setError("فشل في جلب سجل نشاطات العميل");
    } finally {
      setLoading(false);
    }
  };
  ...
  const createdCount = customerLogs.filter(
    (log) => log.action === "created",
  ).length;
  const updatedCount = customerLogs.filter(
    (log) => log.action === "updated",
  ).length;
  const deletedCount = customerLogs.filter(
    (log) => log.action === "deleted",
  ).length;
  const customCount = customerLogs.filter(
  ...
  useEffect(() => {
    ...
    fetchCustomerLogs();
  }, [customerId, currentPage, searchQuery, actionFilter, userData?.token, authLoading]);

  return (
    <div className="flex min-h-screen flex-col">
      <DashboardHeader />
      <div className="flex flex-1 flex-col md:flex-row">
        <EnhancedSidebar activeTab="customers" setActiveTab={() => {}} />
          ...
          <div className="mx-auto">
            ...
            <div className="mb-8">
              ...
              <p className="text-gray-600">
                مراقبة وتتبع جميع الأنشطة المتعلقة بهذا العميل
              </p>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <div className="flex-1">
                    <div className="relative">
                      ...
                      <Input
                        placeholder="البحث في سجل نشاطات العميل..."
                        value={searchQuery}
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  عرض جميع الأنشطة والعمليات المتعلقة بهذا العميل
                </CardDescription>
              </CardHeader>
              <CardContent>
                {loading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <Button onClick={fetchCustomerLogs} variant="outline">
                      إعادة المحاولة
                    </Button>
                  </div>
