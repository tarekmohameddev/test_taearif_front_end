...
import { getDefaultSideBySideData } from "@/context/editorStoreFunctions/sideBySideFunctions";
...
const sideBySide = (props: sideBySideProps = {}) => {
  ...
  const variantId = props.variant || "sideBySide1";
  ...
  const updateSideBySideByPath = useEditorStore(
    (s) => s.updateSideBySideByPath,
  );
  ...
  // Get default data
  const defaultData = getDefaultSideBySideData();
  ...
  if (props.useStore && typeof window !== "undefined" && (window as any).__DEBUG_COMPONENT_DATA__) {
    console.group("🔍 SideBySide Data Sources");
  ...
  const spacingSelector = useMemo(
    () => (state: any) => {
      ...
      const componentData = state.sideBySideStates[uniqueId];
  ...
  const updateSpacing = (newSpacing: {
    ...
    if (props.useStore) {
      updateSideBySideByPath(uniqueId, "spacing", newSpacing);
    }
  };
  ...
  if (typeof window !== "undefined") {
    // @ts-ignore
    window.updateSideBySideSpacing = updateSpacing;

    // Test function to demonstrate reactive spacing
    // @ts-ignore
    window.testSideBySideSpacing = () => {
  ...
  return (
    <section
      ...
      data-debug="sideBySide-component"
    >
      <div
        className={cn(
          ...
          <div className="flex flex-col">
            ...
            >
              {mergedData.content?.title ||
                "نحن شريكك الموثوق في عالم العقارات"}
            </h2>
          </div>
          <p
            className={cn(
