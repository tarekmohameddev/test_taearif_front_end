...
export function CustomersTable() {
  ...
  const renderLastPropertyRequest = (req: LastPropertyRequest | null | undefined) => {
    ...
    return (
      <div className="text-right text-sm space-y-0.5 w-full min-w-0 max-w-[150px]">
        {location && (
          <div className="flex items-center gap-1 justify-start text-gray-700 dark:text-gray-300 w-full min-w-0">
            <MapPin className="h-3.5 w-3.5 text-gray-400 shrink-0" />
            <span className="min-w-0 ">{location}</span>
          </div>
        )}
        {typeLine && (
          <div className="flex items-center gap-1 justify-start text-gray-600 dark:text-gray-400 text-xs w-full min-w-0">
            <Home className="h-3.5 w-3.5 text-gray-400 shrink-0" />
            <span className="min-w-0 ">{typeLine}</span>
          </div>
        )}
      </div>
    );
