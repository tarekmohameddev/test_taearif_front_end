...
## 8) Single-Line Text (سطر واحد فقط — Desktop / Mobile)

- Add an **object** (e.g. under typography or styling for that text) containing **two boolean (switch) fields**:
  - **One switch:** to enable *"جعل النص في سطر واحد فقط وفي row واحد بشكل اجباري في الكمبيوتر"* (force text to a single line / single row on desktop).
...
## 9) Consistency with the Component (إجباري)
