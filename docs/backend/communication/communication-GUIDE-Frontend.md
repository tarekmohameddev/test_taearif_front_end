...
| WhatsApp | `/api/v1/whatsapp/*` | Numbers, conversations, messages, templates, automation, AI config, **campaigns** |
| Email | `/api/v1/email/*` | Campaigns, templates, single send, logs, stats |
...
### 3.7 WhatsApp campaigns

Base path: `/api/v1/whatsapp`. Campaigns send bulk WhatsApp messages via a chosen **wa_number** to customers and/or manual phones. Content is either a **plain message** or a **template** (with optional variables); exactly one is required per campaign.

| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/v1/whatsapp/campaigns` | List (paginated). Query: `status`, `per_page`. |
| GET | `/api/v1/whatsapp/campaigns/{id}` | Get one. |
| POST | `/api/v1/whatsapp/campaigns` | Create. **wa_number_id** required; **message** XOR **template_id** (content contract). |
| PATCH | `/api/v1/whatsapp/campaigns/{id}` | Update (draft/scheduled/paused only). Same content contract. |
| DELETE | `/api/v1/whatsapp/campaigns/{id}` | Delete (draft/scheduled only). |
| POST | `/api/v1/whatsapp/campaigns/{id}/send` | Trigger send (now or scheduled). **Idempotency-Key** required. |
| POST | `/api/v1/whatsapp/campaigns/{id}/pause` | Pause in-progress or scheduled campaign. |
| POST | `/api/v1/whatsapp/campaigns/{id}/resume` | Resume paused campaign. **Idempotency-Key** required. |

**Campaign create/update (content contract):**

- Exactly **one** of **message** (plain text) or **template_id** (FK to `wa_templates`) must be set.
- **422** if both missing: `code: "WA_CAMPAIGN_CONTENT_REQUIRED"`.
- **422** if both present: `code: "WA_CAMPAIGN_CONTENT_CONFLICT"`.
- **wa_number_id** (required): must be a number owned by the tenant (`GET /api/v1/whatsapp/numbers`). When `communication.whatsapp.campaign.require_active_wa_number` is true (default), the number must be **active** to send.

**Campaign list/show response:** Each campaign includes `creator_display` (same shape as SMS), `wa_number_id`, `template_id`, `message`, `status`, `scheduled_at`, `sent_at`, `recipient_count`, `sent_count`, `delivered_count`, `failed_count`, and optional `meta` (e.g. `variables` for template mode).

**Campaign send (POST `/api/v1/whatsapp/campaigns/{id}/send`):**

- **Header:** `Idempotency-Key` is **required** (e.g. `crypto.randomUUID()`). Reuse the same key for retries of the same request.
- **Body:** At least **one** of `customer_ids` (array of integers) or `manual_phones` (array of strings) with at least one value. Phones 8–16 digits.
- **Success (202):** `data` includes `campaign_id`, `status`, `recipient_count`, `dispatch_reference`, `queued_at`, `scheduled_at`.
- **Errors:** 400 (e.g. INSUFFICIENT_CREDITS), 404 (CAMPAIGN_NOT_FOUND or WA_NUMBER_NOT_FOUND), 409 (idempotency conflict), 422 (validation or WA_NUMBER_NOT_ACTIVE, WA_CAMPAIGN_CONTENT_*).

**Pause (POST `/api/v1/whatsapp/campaigns/{id}/pause`):** 200 with `data` (campaign_id, status, sent_count, paused_count, credit_info). Only in-progress or scheduled campaigns can be paused.

**Resume (POST `/api/v1/whatsapp/campaigns/{id}/resume`):** **Idempotency-Key** required. Body: `mode` (required: `continue` | `restart`), optional `customer_ids`, `manual_phones` (for restart). 202 with `data` (campaign_id, status, mode, recipient_count, credit_info).

**Frontend:** Ensure exactly one of `message` or `template_id` when creating/updating; send at least one of `customer_ids` or `manual_phones` when sending; use Idempotency-Key for send and resume; display API error `code` and `message` on failure.
...
| POST | `/api/v1/sms/campaigns/{id}/send` | Trigger send (now or scheduled). **Idempotency-Key required.** |

**Campaign list/show response:** Each campaign includes `creator_display`:
- **Employee:** `{ id, type: "employee", first_name, last_name }`
- **User (tenant):** `{ id, type: "user", company_name }` — `company_name` comes from `user_basic_settings`.

**Campaign send (POST `/api/v1/sms/campaigns/{id}/send`):**

- **Header:** `Idempotency-Key` is **required** (e.g. `crypto.randomUUID()`). Use the same key when retrying the same request so the server does not double-charge.
- **Body:** At least **one** of `customer_ids` (array of integers) or `manual_phones` (array of strings) is **required**, each with at least one value. Phones must be valid (8–16 digits).
- **Success (202):** `data` includes `campaign_id`, `status`, `recipient_count`, `dispatch_reference`, `queued_at`, `scheduled_at`.
- **Replay:** Sending again with the **same** Idempotency-Key and **same** payload returns **202** with the same `data` (no double charge). Different payload with same key returns **409** (hash mismatch).
- **Errors:** 400 (e.g. insufficient credits), 404 (campaign not found), 409 (idempotency conflict / hash mismatch), 422 (validation):
  - Missing Idempotency-Key or missing recipients: *"At least one of customer_ids or manual_phones must be provided with at least one value."*
  - Recipients provided but none valid: *"No valid phone numbers from the given customer_ids or manual_phones. Ensure customer IDs exist and have a valid phone (8–16 digits), and that manual_phones are valid (8–16 digits)."*
- **Frontend:** Always send at least one of `customer_ids` or `manual_phones` with at least one entry; disable Send until the user has selected or entered recipients; display the API 422 message on validation failure.
...
| Method | Path | Description |
|--------|------|-------------|
| POST | `/api/v1/sms/messages/send` | Send single SMS. **Idempotency-Key required.** |
| GET | `/api/v1/sms/logs` | List SMS message logs (paginated). |
| GET | `/api/v1/sms/stats` | SMS stats for tenant. |

**Single SMS send (POST `/api/v1/sms/messages/send`):**

- **Header:** `Idempotency-Key` is **required** (e.g. `crypto.randomUUID()`). Reusing the same key with the same body returns the same result without charging again.
- **Body:** `recipient_phone` (required string, e.g. `+966500000001`), `content` (required string).
- **Success (202):** `data` includes `log_id`, `status` (e.g. `sent`), `recipient_phone`, `gateway_message_id`, `provider`, `sent_at`.
- **Errors:** 400 (INSUFFICIENT_CREDITS), 409 (idempotency conflict), 422 (validation, e.g. missing Idempotency-Key or invalid phone/content), 502 (SMS provider failed; credit is refunded).

---

## 5. Email

Base path: `/api/v1/email`. All endpoints require auth and are tenant-scoped.

### 5.1 Campaigns

| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/v1/email/campaigns` | List (paginated). Query: `status`, `per_page`. |
| GET | `/api/v1/email/campaigns/{id}` | Get one. |
| POST | `/api/v1/email/campaigns` | Create. **subject**, **body_html** required; optional **body_text**, **template_id**. |
| PATCH | `/api/v1/email/campaigns/{id}` | Update (draft/scheduled/paused only). |
| DELETE | `/api/v1/email/campaigns/{id}` | Delete (draft/scheduled only). |
| POST | `/api/v1/email/campaigns/{id}/send` | Trigger send (now or scheduled). **Idempotency-Key** required. |
| POST | `/api/v1/email/campaigns/{id}/pause` | Pause in-progress or scheduled campaign. |
| POST | `/api/v1/email/campaigns/{id}/resume` | Resume paused campaign. **Idempotency-Key** required. |

**Campaign list/show response:** Each campaign includes `creator_display` (same shape as SMS), `subject`, `body_html`, `body_text`, `template_id`, `status`, `scheduled_at`, `sent_at`, `recipient_count`, `sent_count`, `delivered_count`, `failed_count`, and optional `meta`.

**Campaign send (POST `/api/v1/email/campaigns/{id}/send`):**

- **Header:** `Idempotency-Key` is **required** (e.g. `crypto.randomUUID()`). Reuse the same key when retrying the same request.
- **Body:** At least **one** of `customer_ids` (array of integers) or `manual_emails` (array of valid email strings) is **required**, each with at least one value.
- **Success (202):** `data` includes `campaign_id`, `status`, `recipient_count`, `dispatch_reference`, `queued_at`, `scheduled_at`.
- **Replay:** Same Idempotency-Key and same payload returns **202** with the same `data` (no double charge). Different payload with same key returns **409**.
- **Errors:** 400 (e.g. INSUFFICIENT_CREDITS), 404 (campaign not found), 409 (idempotency conflict), 422 (validation):
  - Missing Idempotency-Key or missing recipients: *"At least one of customer_ids or manual_emails must be provided with at least one value."*
  - No valid email addresses from the given customer_ids or manual_emails.
- **Frontend:** Send at least one of `customer_ids` or `manual_emails` with at least one entry; use Idempotency-Key for send and resume; display API error message on failure.

**Pause (POST `/api/v1/email/campaigns/{id}/pause`):** 200 with `data` (campaign_id, status, sent_count, paused_count, credit_info). Only in-progress or scheduled campaigns can be paused.

**Resume (POST `/api/v1/email/campaigns/{id}/resume`):** **Idempotency-Key** required. Body: `mode` (required: `continue` | `restart`), optional `customer_ids`, `manual_emails` (for restart). 202 with `data` (campaign_id, status, mode, recipient_count, credit_info).
...
| PATCH | `/api/v1/email/templates/{id}` | Update. |
| DELETE | `/api/v1/email/templates/{id}` | Delete. |

### 5.3 Single message and logs/stats

| Method | Path | Description |
|--------|------|-------------|
| POST | `/api/v1/email/messages/send` | Send single email. **Idempotency-Key** required. |
| GET | `/api/v1/email/logs` | List email message logs (paginated). Query: e.g. `campaign_id`. |
| GET | `/api/v1/email/stats` | Email stats for tenant. |

---

## 6. Communication ops (reliability)
...
- **WhatsApp campaign send (AI-agent workflow):** [whatsapp/wa-campaign-send-workflow-AI.md](./whatsapp/wa-campaign-send-workflow-AI.md) — step-by-step workflow for sending a WhatsApp campaign.
- **Email campaign send (AI-agent workflow):** [email/email-campaign-send-workflow-AI.md](./email/email-campaign-send-workflow-AI.md) — step-by-step workflow for sending an email campaign.
- **SMS campaign send (AI-agent workflow):** [sms/sms-campaign-send-workflow-AI.md](./sms/sms-campaign-send-workflow-AI.md) — step-by-step, AI-friendly workflow for listing customers and sending a campaign.
- Backend route definitions live in `routes/api.php` (prefix `v1` for conversations, messages, credits, whatsapp, sms, email, communication/ops).
